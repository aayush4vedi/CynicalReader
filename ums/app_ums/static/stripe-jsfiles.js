...
function stripeElements(publishableKey) {
  ...
  if (signupForm) {
    signupForm.addEventListener('submit', function (evt) {
      ...
      createCustomer().then((result) => {
        ...
        console.log("custome created: ",customer);
        window.location.href = '/prices/' + customer.id;
      });
    });
  }
  ...
  if (paymentForm) {
    paymentForm.addEventListener('submit', function (evt) {
        ...
        // alert("paymentForm@submit latestInvoicePaymentIntentStatus: ",latestInvoicePaymentIntentStatus);
...
function createPaymentMethod({ card, isPaymentRetry, invoiceId }) {
  ...
  stripe
    .createPaymentMethod({
      ...
      card: card,
      billing_details: {
        name: billingName,  //NOTE: sending address here is a must; TODO: ask address from customer
        address: {
...
function switchPrices(newPriceIdSelected) {
    ...
    const currentSubscribedpriceId = priceId;
    // alert("currentSubscribedpriceId:"+currentSubscribedpriceId);
    // alert("currentSubscribedpriceId:"+newPriceIdSelected);
//   const customerId = params.get('customerId');
//   const subscriptionId = params.get('subscriptionId');
//NOTE: already set in account-stripe initial script.TODO: remove this hack
  // Update the border to show which price is selected
...
//   alert("newPriceId:"+newPriceId);
  ...
  updateSubscription(newPriceId.toUpperCase(), subscriptionId).then(
    ...
    //   alert("searchParams:"+searchParams.toString());
    //   window.location.search = searchParams.toString();
        // update priceId & priceHasChanged in url
      window.location.href =
        '/account/' +
        subscriptionId +
        '/' +
        newPriceId.toUpperCase() +
        '/' +
        currentPeriodEnd +
        '/' +
        customerId +
...
function createCustomer() {
  ...
  console.log("\t@createCustomer..... body: ",reqbody);
  return fetch('/signup', {
        method: 'post',
        headers: {
...
function getFormattedAmount(amount) {
  ...
  function getCustomersPaymentMethod() {
    ...
    if (paymentMethodId) {
      retrieveCustomerPaymentMethod(paymentMethodId).then(function (response) {
        ...
        document.getElementById(
          'subscribed-price'
        ).innerText = capitalizeFirstLetter(priceId);               //NOTE: already set in account-stripe initial script.TODO: remove this hack
  ...
  function demoChangePrice() {
    ...
    priceIdLower = priceId.toLowerCase();
    //NOTE: already set in account-stripe initial script.TODO: remove this hack
