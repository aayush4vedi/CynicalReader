...
CS_ASYNC_ITEM_SCRAPED = [0,0]                               # NOTE: [seedha,ulta] Successfully hit url & get content with async method
CS_ASYNC_URL_UNREACHABLE = [0,0]                            # NOTE: [seedha,ulta] Url unreachable after retries with async
CS_ASYNC_SEMA_EXCEPTION_ERR = [0,0]                         # NOTE: [seedha,ulta] Tried-Catch error when running async jobs with semaphore
CS_ITEMS_WRITTEN_DIRECT = 0                                 # No need to scrape item with asycn, content already present
CS_BOYS_STILL_PLAYING = [0,0]                               # NOTE: [seedha,ulta] Items sent for async which are not returned yet
CS_BOYS_PLAYING_LIMIT = 50                                  # used in if gw.CS_BOYS_STILL_PLAYING % gw.CS_BOYS_PLAYING_LIMIT == 0: time.sleep(2)    
